...
/**
 * An evaluator for calculating rimu! flashlight skill.
 * <br><br>
 * This class should be considered an "evaluating" class and not persisted.
 */
public final class RimuFlashlightEvaluator {
    private RimuFlashlightEvaluator() {
    ...
    public static double evaluateDifficultyOf(DifficultyHitObject current, boolean isHiddenMod, boolean withSliders) {
        ...
        double scalingFactor =
                52 / current.object.getRadius(GameMode.rimu);
        ...
        for (int i = 0; i < Math.min(current.index, 10); ++i) {
            ...
            // Exclude overlapping objects that can be tapped at once.
            if (!(currentObject.object instanceof Spinner) && !currentObject.isOverlapping(false)) {
                double jumpDistance = current.object
                        .getStackedPosition(GameMode.rimu)
                        .subtract(currentObject.object.getStackedEndPosition(GameMode.rimu))
                        .getLength();
