...
/**
 * An evaluator for calculating rimu! aim skill.
 * <br><br>
 * This class should be considered an "evaluating" class and not persisted.
 */
public final class RimuAimEvaluator {
    private static final double wideAngleMultiplier = 1.65;
    private static final double acuteAngleMultiplier = 1.95;
    private static final double sliderMultiplier = 1.5;
    private static final double velocityChangeMultiplier = 0.85;

    private static final double singleSpacingThreshold = 125;
    private static final double minSpeedBonus = 150;

    private RimuAimEvaluator() {
    ...
    /**
     * Calculates the aim strain of a hit object.
     */
    ...
    /**
     * Calculates the movement strain of a hit object.
     */
    private static double movementStrainOf(DifficultyHitObject current) {
        double speedBonus = 1;

        if (current.strainTime < minSpeedBonus) {
            speedBonus += 0.75 * Math.pow((minSpeedBonus - current.strainTime) / 45, 2);
        }

        DifficultyHitObject previous = current.previous(0);
        double travelDistance = previous != null ? previous.travelDistance : 0;
        double distance = Math.min(singleSpacingThreshold, travelDistance + current.minimumJumpDistance);

        return 50 * speedBonus *  Math.pow(distance / singleSpacingThreshold, 5) / current.strainTime;
    }
