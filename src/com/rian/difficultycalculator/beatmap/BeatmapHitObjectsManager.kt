 ...
 */
class BeatmapHitObjectsManager : Cloneable {
    /**
     * All objects in this beatmap.
     */
    private val objects = mutableListOf<HitObject>()
    ...
    fun remove(index: Int): HitObject? {
        ...
        return objects.removeAt(index).apply {
            when (this) {
                is HitCircle -> --circleCount
                is Slider -> --sliderCount
                else -> --spinnerCount
    ...
    public override fun clone() =
        (super.clone() as BeatmapHitObjectsManager).apply {
            this@BeatmapHitObjectsManager.objects.forEach { add(it) }
        }
}
