...
object BeatmapHitObjectsParser : BeatmapSectionParser() {
        ...
        .let {
            ...
            val obj = when {
                ...
                type === HitObjectType.Slider || type === HitObjectType.SliderNewCombo ->
                    createSlider(beatmap, time, position, it)

                type === HitObjectType.Spinner ->
                    createSpinner(beatmap, time, it)
    ...
    private fun createSlider(beatmap: Beatmap, time: Double, startPosition: Vector2, pars: List<String>): Slider {
        if (pars.size < 8) {
        ...
        val timingControlPoint = beatmap.controlPoints.timing.controlPointAt(time)
        ...
        return Slider(
            ...
            if (beatmap.formatVersion < 8) 1 / difficultyControlPoint.speedMultiplier else 1.0
        )
    }

    private fun createSpinner(beatmap: Beatmap, time: Double, pars: List<String>) =
        Spinner(time, beatmap.getOffsetTime(parseInt(pars[5])).toDouble())
}
